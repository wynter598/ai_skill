...
- ✅ AS 关键字对齐：整块统一（闸门极差 ≤ 50）或短/中/长分级；分档与右端与闸门同源（见 §2.2）
...
**每组内 AS 垂直对齐**；**不同组 AS 可以在不同列**，避免短档与长档之间出现过多空白。与 `sql_aligner.py` 行为一致时，不得再用手工「只数 stripped 表达式」去推翻工具分档。

**整块单列（闸门）**：同一 SELECT 块选列区内（整行 `--` 除外），按与下面 **分档长度** 相同的「句首 ref（1-based 垂直续行）→ 列别名 `as` 前最后非空」逐行量**字符长**（非续行头行会更新 ref）；若 `max − min ≤ SELECT_AS_UNIFY_HEAD_CHAR_SPAN_MAX`（默认 **50**），则**不再分短/中/长**，该块所有列别名 `as` 对齐到同一列。

**分级标准**（仅当不满足上述整块条件时；阈值与工具常量 `SHORT_FIELD_MAX` / `MEDIUM_FIELD_MAX` 一致）：
- **短字段组**：分档长度 `tier_len` ≤ 50
- **中字段组**：50 < `tier_len` ≤ 100
- **长字段组**：`tier_len` > 100

**分档长度 `tier_len`（与闸门同源，工具实现 `_field_gate_rel_display_width`）**：
- 在**含有列别名** `\s+as\s+\w` 的那一行（`cast(x as type)` 内的 `as` 排除），从**当前句首 ref** 起至 **`as` 前最后一个非空字符**止的 **显示宽度**（`get_width`，含 CJK 比例）。
- **头行**：`select` / `group by` 起首，或 strip 以 `,` 起首——ref 为关键字后首字段内容列，或逗号所在列（与闸门 `_select_list_line_char_len_unify_gate` 一致）。
- **续行**：ref 沿上一头行垂直传递；故**跨行字段**若 `as` 写在续行上，`tier_len` 量的是「从公共 ref 列到该行 `as` 前」的宽度，**不会**把仅续行上的短片段误当成整条字段长度而归入短档。
- 人工核对简单单行时，可近似理解为「从与首字段对齐列起算到 `as` 前的可视跨度」；与「去掉缩进逗号后只数表达式字符」在多数单行场景接近，但**以工具为准**。

**组内 AS 目标列（工具 `calculate_target_as_column`）**：同组取各字段 **`gate_abs_end`**（行首至列别名 `as` 前最后非空的显示宽度）的 **max**，再加 `AS_SPACING`（默认 1）。与「`ref` 起点 + `tier_len`」在同一行上数学一致；续行字段的右端仍按**物理行**到 `as` 前计量。

**对齐算法**（针对每个 SELECT 语句，与工具一致）：

1. **是否整块统一**：若闸门极差 ≤ 50，则整块一次对齐；否则进入短/中/长。
2. **字段分组**：按 `tier_len` 分为短、中、长三组（解析不到闸门宽度时回退为旧 `field_len`，见源码 `analyze_select_block`）。
3. **组内对齐**：每组 `target_col = max(gate_abs_end) + AS_SPACING`，再对该组字段执行空格填充，使 `as` 同列。
   - **重要**：同组内所有字段的 AS 必须对齐在同一列，不得参差不齐
4. **CASE WHEN 与 `as`**：列式排版以 §2.3 为准；**分档与目标列**仍以含列别名 `as` 的物理行为闸门计量（例如仅 **`end as`** 独占短行时 `tier_len` 会较短）。不要用手数「整条 CASE 字符数」替代工具。
5. **字段排序建议**：先短字段组，再中字段组，最后长字段组
6. **组间位置**：
   - 短字段组AS通常在第20-35列
   - 中字段组AS通常在第60-85列
   - 长字段组AS通常在第85-120列
...
**关键要点**：
1. ✅ 同一分档组内的 AS 必须垂直对齐（形成直线）
2. ✅ 不同分档组的 AS 可以在不同列（避免浪费空间）；整块统一时全块一列
3. ✅ 每个子查询独立应用 §2.2（含闸门与分档）
4. ❌ 不要让同组内 AS 参差不齐
5. ❌ 不要强制所有字段 AS 在同一列（除非闸门触发整块统一；否则易造成短行过多垫空格）
...
8. **子查询内部的列别名对齐遵循 2.2 节规范**：每个子查询独立应用 §2.2 规则（整块闸门或按 `tier_len` 分档），每组内 AS 对齐
