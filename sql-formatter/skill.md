...
**规则**：
1. **子查询起始左括号 `(` 必须单独一行**，并与前方最近的 `from`/`join`/`left join`/`right join` 等关键字的首字母垂直对齐
   - **禁止** `from (` 或 `join (` 等关键字与括号写在同一行
   - **正确** 写法：`from` 单独一行，下一行写 `(`
2. **子查询内部语句另起一行，使用 6 个空格作为统一缩进**（tab = 6 空格）
3. **子查询内 `select` 后直接紧跟第一个查询字段**，不换行
4. **子查询内**若 `from` 后接**物理表**，则 **`from` 与表名同一行**；若 `from` 后接**子查询 `(`**，则 **`from` 单独一行，下一行 `(`**（与规则 1 一致）
5. **子查询闭合右括号 `)` 单独一行**，与起始左括号 `(` 垂直对齐
6. **子查询闭合括号后的 ON**：`on` **单独一行**，缩进与**同层 `left join` / `from`** 首列对齐（见 2.4，**不与 `)` 对齐**）；多个条件时 `and` 换行并与 `on` 同列
7. **子查询内部的列别名对齐遵循 2.2 节规范**：每个子查询独立应用分级对齐规则，按字段长度分组，每组内AS对齐
8. **子查询内行首逗号列（工具行为）**：`sql_aligner.py` 在 `align_subquery_brackets` 中，对子查询括号内、**同一嵌套层**且以 **`,`** 开头的字段行，其**逗号所在列**与同块 **`select`（或 `group by`）首行首字段** 对齐（与 `align_field_names` 的 `field_start_pos - 2` 一致）；**不**再沿用「相对 `min_indent`」以免 `, case when …` 比 `, col as …` 多缩进 1 格。
9. **`select` 列表内多行 `case` 后以独占行 `)` 收尾**：除子查询内外，凡 **`align_field_names` 扫描到的列表**，在 `, … case when … end` 与 **`) as col`** 之间须保持 CASE 块状态直至 **`)`** 行；子句 **`from`/`join`/…** 须能结束扫描。详见 **Issue #4**（`docs/KNOWN_ISSUES.md`）。
